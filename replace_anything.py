...
def replace_anything(
    ...
    # 2. Replace the background with Stable Diffusion
    for idx, mask in enumerate(masks):
        # Set the seed if provided
        if seed is not None:
            torch.manual_seed(seed)

        # Replace the image with the mask
        mask_path = out_dir / f"mask_{idx}.png"
        img_replaced_path = out_dir / f"replaced_with_{Path(mask_path).name}"
        img_replaced = replace_img_with_sd(img, mask, text_prompt, device=device)

        # Save the replaced image
        save_array_to_img(img_replaced, img_replaced_path)
